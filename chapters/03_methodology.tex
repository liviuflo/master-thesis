...
At every iteration step, the algorithm generates a set of correspondence hypotheses --- a heuristic approximation of true correspondences --- by finding the closest point from $Q$ to each point in $P'$. The assumption is that the alignment of the two point clouds is improved by minimizing the distance associated with each correspondence. Depending on how the distance metric is defined, the algorithm can focus on different characteristics of the point clouds.

The simplest formulation uses a point-to-point distance metric.
...
\begin{equation}
	...
	\comma
\end{equation}
where $\kappa$ is a custom scaling factor, set to 0.11 in our experiments.
...
We use the point-to-point distance metric, so the optimization problem becomes:
