...
The inverse operations are $\text{log}:  \SO{3} \rightarrow \so{3}$ and $\text{Log} :\SO{3} \rightarrow \RR^3$, respectively.
...
This can also be formulated as a generic optimization problem, by defining a distance measure between a pose $\pose_{}$ and an uncertain pose $\Mtx{}$, with covariance $\CovM$:

\newcommand{\customres}{\Log{\Mtx{}^{-1} \pose_{}}}

\[
	\text{d}\left(\pose_{}, \Mtx{}, \CovM\right) = \left(\customres\right)^T \cdot \CovM^{-1} \cdot \customres
\]
