...
\begin{equation}
	...
	\eqdot
\end{equation}
As this is intractable on the non-linear manifold $\SE{3}$, an approximation of the solution can be computed using the Weiszfeld algorithm (\ref{alg:weiszfeld}). We disregard the scalar weights $\betaM$, as they can be accounted for, numerically, in the covariances $\CovM$.
...
\begin{algorithm}
	...
	\begin{compactitem}
		...
		\item $\pose_0$: pose to linearize around
	...
	\begin{algorithmic}[1] % number every line
		\State $\xi \leftarrow \vecx{0}_{6\times 1}$
		\For{$i=1, \dots, \iterth$}
		\State $\pose \leftarrow \pose_0\Expmap{\xi}$
		\State $w_{\xi}  = \sum_{\Mtx{}}\Logmap{\pose^{-1} \Mtx{}}/\dfunc{\pose, \Mtx{}, \CovM}$
		\State $w  = \sum_{\Mtx{}}1/\dfunc{\pose, \Mtx{}, \CovM}$
		\State $\xinew = w_{\xi} / w$
		\If{$\normtwo{\xinew - \xi} < 10^{-7}$}
		\State \textbf{break} \Comment{Early stop, no significant improvement occurred}
		\EndIf
		\State $\xi \leftarrow \xinew$
		\EndFor
...
we can perform \acrfull{svd} $\matx{U}\matx{D}\transpose{\matx{V}}=\sum_{i} w_i \matx{R}_i$. The \emph{chordal mean} \cite{hartley2013rotation} is then obtained as \mbox{$\bar{\matx{R}}=\matx{U}\transpose{\matx{V}}$}. The scalar weight must be approximated from the covariance matrix, \eg $w_i = \text{tr}\left(\matx{\Sigma}_{\matx{R}_i}\right)$. For translations, we exploit the linearity and compute \mbox{$
		\bar{\vecx{t}}=
		\left(\sum_{\vectm}\matx{\Sigma}_{\vectm}^{-1}\right)^{-1}
		\sum_{\vectm}\matx{\Sigma}_{\vectm}^{-1}\vectm
	$}.

These there approaches have been compared by generating large sets of poses with known covariance around an arbitrary reference pose (treated as the ground truth mean), and assessing how the predicted mean differs from it. The results of this experiment are presented in Fig.~\ref{fig:pose-averaging}.

\begin{figure}[h]
	\centering
	\subcaptionbox{Translation averaging error distribution}{
		\includegraphics[width=0.45\linewidth]{images/pose_avg_trans_100s.pdf}
	}
	\hspace{1pt}
	\subcaptionbox{Rotation averaging error distribution}{
		\includegraphics[width=0.45\linewidth]{images/pose_avg_rot_100s.pdf}
	}
	\caption[Rotation and translation averaging experiment]{}
