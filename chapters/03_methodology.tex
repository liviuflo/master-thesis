...
At its core, the odometry and mapping solution that we propose requires minimal input: a sequence of (motion-compensated) point clouds captured by a moving LiDAR scanner, and a timestamp for each scan. If available, the output of a localization or INS sensor can be employed to provide absolute constraints, but this is not a strict requirement.

Let $\mathfrak{P} = \left\{ P_0, P_1, ... \right\}$ represent the set of point clouds that we operate on, and $\mathfrak{T} = \left\{ \gtposei{0}, \gtposei{1}, ... \right\}$ the ground truth poses at which each scan was captured. In an ideal setting, a point cloud registration algorithm would take $P_i$ and $P_{i+1}$ as input and return the transformation $\Delta\matx{T}_i = \gtposei{i}^{-1} \gtposei{i+1}$, the ground truth displacement between consecutive poses. This would correspond to a perfect odometry model, and the trajectory could be reconstructed by accumulating the computed displacement. Perhaps with the exception of some simulation environments, such an approach is actually not feasbile. As we have already seen, LiDAR output is not perfect, especially in dynamic scenes, and some scenarios are simply unsuitable for odometry estimation based on 3D features \cite{lidartunnel}.
