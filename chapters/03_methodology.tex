...
Moreover, we apply the Geman-McClure function \reffig{robust-residual} on the point-to-point distance metric, like in \cite{vizzo2023ral}, which diminishes the influence of large residuals during minimization. For a residual $r$, this is defined as:
\begin{equation}
...
We have also experimented with approaches for removing points that could skew registration results, such as those originating from regions occupied by trees or bushes. These regions are not necessarily sparse, so filtering based on the distance to the nearest neighbor or a related statistical measure was not very effective.
...
A low entropy value indicates a structured, well-formed region (\eg points on a planar surface), while high entropy is usually associated with noisy regions. \reffig{sub-entr-view}
The size of the neighborhood, defined as a radius around each point, was set to 0.3m.
Filtering discards points with entropy above a certain percentile, to avoid a fixed threshold that would not be able to adjust to variable scene features. \reffig{sub-entr-log}
...
\begin{figure}
	...
	\subcaptionbox{Per-scan entropy across a section of the dataset. Entropy values vary depending on the scene features, making it difficult to find a satisfactory global threshold.\label{fig:sub-entr-log}}{
		\includegraphics[width=0.48\linewidth]{images/entropy-log.png}
	}
